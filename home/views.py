...
def data_analysis(data_main, lr, iter):
    ...
    for i in range(iterations):
        ...
        a = np.dot(X, theta)-y
        error = np.power(a, 2)
        mse = np.sum(error)*(1/m)
        a = np.dot(X.transpose(), a)

        b = a * (alpha/m)
        theta  = theta - b
        #plt.scatter(x, y)
        #plt.plot(x, theta[0][0] + theta[1][0]*x)
        error_list.append([i,mse])
    
    theta_list = [[0,*theta[0]],[20, *(theta[0]+(theta[1]*20))]]
    
    #print(ans)
    #plt.scatter(x, y)
    #plt.plot(x, ans[0][0] + ans[1][0]*x)
    ...
    for j,k in zip(x1, y1):
        scatter_list.append([*j, *k])

    return error_list, scatter_list, theta_list
